...
func TestJWTer_GetToken(t *testing.T) {
	...
	want, err := jwt.NewBuilder().
		...
		Issuer(`github.com/litencatt/go_todo_app`).
		Subject("acces_token").
		IssuedAt(c.Now()).
	...
	// 作成したアクセストークンを使ったHTTPリクエストの作成
	req := httptest.NewRequest(http.MethodGet, `https://github.com/litencatt`, nil)
